...
public class InteractionInspector extends BaseInspector {
    ...
    public void performEntityInteractionLookup(final Player player, final Entity entity) {
        class BasicThread implements Runnable {
            @Override
            public void run() {
                try {
                    ...
                    try (Connection connection = getDatabaseConnection(player)) {
                        Statement statement = connection.createStatement();
                        String blockData = InteractionLookup.performEntityLookup(null, statement, entity.getLocation().getBlock(), entity.getUniqueId().toString(), player, 0, 1, 7);
